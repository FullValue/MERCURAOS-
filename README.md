...
1. **Registre des coûts → Référentiel Mercura** : créer les formats (volume en ml), parfums (prix liquide initial HT/L) et références (SKU propre, SKU Shopify facultatif). Les références peuvent être désactivées et les SKU modifiés.
2. Créer une période datée dans le registre. Renseigner le prix liquide par parfum, les composants par format, leur coût HT/TTC, quantité et classement éventuel en façonnage. Renseigner le coût fixe de série, coût variable et quantité de lot de chaque format, puis le taux de perte, la livraison, les coûts variables fournisseur et la TVA récupérable.
